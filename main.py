...
#         st.warning("Are you sure? Click again to confirm clearing all habits and logs.")
   ...
   if 'db_initialized' not in st.session_state:
       ...
       habits_has_user_id = False
       if 'habits' in existing_tables:
           columns = {col['name'] for col in inspector.get_columns('habits')}
           habits_has_user_id = 'user_id' in columns

       # Drop and recreate tables if schema is outdated or tables are missing
       if not required_tables.issubset(existing_tables) or not habits_has_user_id:
           Base.metadata.drop_all(engine)  # Drop all tables
           Base.metadata.create_all(engine)  # Recreate with current schema
